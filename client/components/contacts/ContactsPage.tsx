...
export default function ContactsPage({ currentUser, friends, onOpenChat }: Props) {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden" style={{ background: "var(--color-bg)" }}>
      {/* Header */}
      <div className="px-6 pt-6 pb-4 shrink-0" style={{ background: "var(--color-surface)", borderBottom: "1px solid var(--color-s3)" }}>
        <h2 className="font-bold text-lg mb-4">Danh bạ</h2>

        {/* Tabs */}
        <div className="flex gap-1 p-1 rounded-2xl" style={{ background: "var(--color-s2)", width: "fit-content" }}>
          {([
            { id: "friends",  label: "Bạn bè" },
            { id: "requests", label: `Lời mời${friendRequests.length ? ` (${friendRequests.length})` : ""}` },
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {tab === "friends" && (
          <div className="grid grid-cols-1 gap-3 max-w-2xl">
            {friends.length === 0 && <Empty icon="👥" text="Chưa có bạn bè nào" />}
            {friends.map((f) => (
              <UserCard
                key={f._id}
                user={f}
                actions={[
                  { label: "Nhắn tin",  primary: true,  onClick: () => onOpenChat(f._id) },
                  { label: "Huỷ bạn",   danger:  true,  onClick: () => handleUnfriend(f._id) },
                ]}
              />
            ))}
          </div>
        ...
        {tab === "requests" && (
          <div className="grid grid-cols-1 gap-3 max-w-2xl">
            {friendRequests.length === 0 && <Empty icon="💌" text="Không có lời mời nào" />}
            {friendRequests.map((u) => (
              <UserCard
                key={u._id}
                user={u}
                actions={[
                  { label: "Chấp nhận", primary: true,  onClick: () => handleAccept(u._id) },
                  { label: "Từ chối",   danger:  true,  onClick: () => handleReject(u._id) },
                ]}
              />
            ))}
          </div>
        ...
        {tab === "search" && (
          <div className="max-w-2xl">
            <div className="flex gap-2 mb-5">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  style={{ background: "var(--color-surface)", border: "1px solid var(--color-s3)" }}
                />
              </div>
              <button
            ...
            <div className="grid grid-cols-1 gap-3">
              {results.length === 0 && query && !searching && <Empty icon="🔍" text="Không tìm thấy kết quả" />}
              {results.map((u) => {
                ...
                return (
                  <UserCard
                    key={u._id}
                    user={u}
                    actions={isSelf ? [] : isFriend
                      ? [{ label: "Nhắn tin", primary: true, onClick: () => onOpenChat(u._id) }]
                      : u.friendStatus === "sent"
                        ? [{ label: "Đã gửi lời mời", disabled: true, onClick: () => {} }]
                        : [{ label: "Kết bạn", primary: true, onClick: () => handleAddFriend(u._id) }]
                    }
                  />
                );
              })}
      ...
      <div className="flex gap-2 shrink-0">
        {actions.map((a) => (
          <button
            ...
            style={{
              background: a.primary ? "var(--color-brand)"
                        : a.danger  ? "#FEF2F2"
                        : "var(--color-s2)",
              color:      a.primary ? "white"
                        : a.danger  ? "var(--color-danger)"
                        : "var(--color-ink-2)",
            }}
          >
            {a.label}
...
function Empty({ icon, text }: { icon: string; text: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 gap-3">
      ...
      <p className="text-sm" style={{ color: "var(--color-ink-4)" }}>{text}</p>
    </div>
  );
}
