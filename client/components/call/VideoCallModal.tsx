...
export default function VideoCallModal({ currentUser }: Props) {
  const { status, callType, incoming, peerSocketId, convId, isMuted, isCamOff, reset } = useCallStore();
  const localRef  = useRef<HTMLVideoElement>(null);
  const remoteRef = useRef<HTMLVideoElement>(null);
  const { startCall, endCall, toggleMute, toggleCam } = useWebRTC(localRef, remoteRef);
  ...
  const reject = () => {
    if (incoming)
      getSocket()?.emit("call:reject", { callerSocketId: incoming.callerSocketId });
    endCall(peerSocketId ?? undefined, convId ?? undefined, elapsed);
  };
  ...
  if (status === "ringing" && incoming) {
    return (
      <div
        ...
        style={{ background: "var(--color-surface)", boxShadow: "var(--shadow-lg)", minWidth: 280 }}
      >
        {/* Pulse avatar */}
        <div className="relative shrink-0">
          <div className="absolute inset-0 rounded-full animate-ripple"
            style={{ background: "var(--color-brand)", opacity: .3 }} />
          <img
            src={incoming.from.avatar}
            className="relative z-10 w-12 h-12 rounded-full object-cover"
            alt={incoming.from.fullName}
          />
        </div>
        <div className="flex-1 min-w-0">
          <p className="font-bold text-sm truncate">{incoming.from.fullName}</p>
          <p className="text-xs" style={{ color: "var(--color-ink-3)" }}>
            {incoming.callType === "video" ? "📹 Cuộc gọi video" : "📞 Cuộc gọi thoại"} đến
          </p>
        </div>
        <div className="flex gap-2 shrink-0">
          <RoundBtn color="var(--color-danger)" title="Từ chối" onClick={reject}>
            <PhoneOffIcon className="w-5 h-5" />
          </RoundBtn>
          <RoundBtn color="var(--color-online)" title="Chấp nhận" onClick={accept}>
    ...
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center animate-fade-in"
        ...
        <div className="relative mb-8">
          <div className="absolute inset-0 rounded-full animate-ripple" style={{ background: "var(--color-brand)", opacity: .2 }} />
          <div className="absolute inset-0 rounded-full animate-ripple" style={{ background: "var(--color-brand)", opacity: .15, animationDelay: ".5s" }} />
          <img
            src={incoming?.from.avatar || ""}
  ...
  /* ═══ 3. Connected ═══ */
  return (
    <div className="fixed inset-0 z-50 flex flex-col animate-fade-in" style={{ background: "#0D0E14" }}>
      {/* Remote video */}
      ...
      <div className="relative z-20 flex items-center justify-between px-6 pt-6 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-white font-semibold text-sm">{currentUser.fullName}</p>
            <p className="text-white/50 text-xs">{fmt(elapsed)}</p>
          </div>
        </div>
        <span className="text-white/50 text-xs px-2 py-1 rounded-full"
      ...
      <div className="relative z-20 flex items-center justify-center gap-5 pb-10">
        ...
        {callType === "video" && (
          ...
          >
            {isCamOff ? <CamOffIcon className="w-5 h-5" /> : <CamIcon className="w-5 h-5" />}
          </ControlBtn>
        )}
        ...
        <div className="flex flex-col items-center gap-1">
          <button
            onClick={() => endCall(peerSocketId ?? undefined, convId ?? undefined, elapsed)}
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <button
        ...
        style={{ background: active ? "rgba(239,68,68,.5)" : "rgba(255,255,255,.15)", color: "white" }}
      >
        {children}
      </button>
...
/* ── Icons ───────────────────────────────────────── */
const PhoneIcon    = ({ className }: { className: string }) => <svg className={className} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>;
const PhoneOffIcon = ({ className }: { className: string }) => <svg className={className} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" d="M16.5 12.5l-1.2 1.2a11 11 0 01-4.95-4.95l1.2-1.2m2.09-3.93A10 10 0 1 1 4.5 19.5"/><path strokeLinecap="round" d="M3 3l18 18"/></svg>;
const MicIcon      = ({ className }: { className: string }) => <svg className={className} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" d="M12 1a3 3 0 00-3 3v8a3 3 0 006 0V4a3 3 0 00-3-3zM19 10v2a7 7 0 01-14 0v-2M12 19v4m-4 0h8"/></svg>;
const MicOffIcon   = ({ className }: { className: string }) => <svg className={className} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" d="M1 1l22 22M9 9v3a3 3 0 005.12 2.12M15 9.34V4a3 3 0 00-5.94-.6M17 16.95A7 7 0 015 12v-2m14 0v2a7 7 0 01-.11 1.23M12 19v4m-4 0h8"/></svg>;
const CamIcon      = ({ className }: { className: string }) => <svg className={className} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" d="M23 7l-7 5 7 5V7zM1 5a2 2 0 012-2h14a2 2 0 012 2v14a2 2 0 01-2 2H3a2 2 0 01-2-2V5z"/></svg>;
const CamOffIcon   = ({ className }: { className: string }) => <svg className={className} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" d="M16 16v1a2 2 0 01-2 2H3a2 2 0 01-2-2V7a2 2 0 012-2h2m5.66 0H14a2 2 0 012 2v3.34l1 1L23 7v10M1 1l22 22"/></svg>;
