...
export default function ChatInput({ convId, onSend, disabled }: Props) {
  ...
  return (
    <div style={{ background: "var(--color-surface)", borderTop: "1px solid var(--color-s3)" }}>
      {/* Reply bar */}
      {replyingTo && (
        <div className="flex items-center gap-3 px-4 py-2" style={{ borderBottom: "1px solid var(--color-s3)" }}>
          ...
          <button onClick={() => setReplyTo(null)} className="shrink-0" style={{ color: "var(--color-ink-4)" }}>
      ...
      {/* File previews */}
      {files.length > 0 && (
        <div className="flex gap-2 px-4 py-2 overflow-x-auto" style={{ borderBottom: "1px solid var(--color-s3)" }}>
          {files.map((f, i) => (
            <div key={i} className="relative shrink-0 group">
              {f.type.startsWith("image/") ? (
                ...
                <div className="w-16 h-16 rounded-xl flex flex-col items-center justify-center gap-1"
                  ...
                  <FileIcon className="w-5 h-5" style={{ color: "var(--color-brand)" }} />
                  <span className="text-[9px] text-center truncate w-12 px-1" style={{ color: "var(--color-ink-3)" }}>
                    {f.name}
                  </span>
                </div>
              )}
      ...
      <div className="flex items-end gap-2 px-3 py-2">
        ...
        <div className="flex items-center gap-0.5 shrink-0 pb-1">
          <ToolBtn title="Emoji" onClick={() => {}}>😊</ToolBtn>
          <ToolBtn title="Hình ảnh" onClick={() => imgRef.current?.click()}>
            <ImageIcon className="w-5 h-5" />
          </ToolBtn>
          <ToolBtn title="Tệp" onClick={() => fileRef.current?.click()}>
        ...
        <button
          ...
          className="shrink-0 w-10 h-10 mb-0.5 rounded-2xl flex items-center justify-center transition-all active:scale-95 disabled:opacity-40"
          style={{ background: text.trim() || files.length ? "var(--color-brand)" : "var(--color-s3)" }}
        >
          {sending
            ? <SpinIcon className="w-4 h-4 text-white animate-spin" />
            : text.trim() || files.length
              ? <SendIcon className="w-4 h-4 text-white" />
              : <ThumbsIcon className="w-5 h-5" style={{ color: "var(--color-ink-3)" }} />
          }
        </button>
...
const SendIcon   = ({ className }: { className: string }) => <svg className={className} fill="currentColor" viewBox="0 0 20 20"><path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z"/></svg>;
...
const ImageIcon  = ({ className }: { className: string }) => <svg className={className} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>;
const AttachIcon = ({ className }: { className: string }) => <svg className={className} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" d="M21.44 11.05l-9.19 9.19a6 6 0 01-8.49-8.49l9.19-9.19a4 4 0 015.66 5.66l-9.2 9.19a2 2 0 01-2.83-2.83l8.49-8.48"/></svg>;
const FileIcon   = ({ className, style }: { className: string; style?: React.CSSProperties }) => <svg className={className} style={style} fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clipRule="evenodd"/></svg>;
const ThumbsIcon = ({ className, style }: { className: string; style?: React.CSSProperties }) => <svg className={className} style={style} fill="currentColor" viewBox="0 0 20 20"><path d="M2 10.5a1.5 1.5 0 113 0v6a1.5 1.5 0 01-3 0v-6zM6 10.333v5.43a2 2 0 001.106 1.79l.05.025A4 4 0 008.943 18h5.416a2 2 0 001.962-1.608l1.2-6A2 2 0 0015.56 8H12V4a2 2 0 00-2-2 1 1 0 00-1 1v.667a4 4 0 01-.8 2.4L6.8 7.933a4 4 0 00-.8 2.4z"/></svg>;
