...
export default function ChatWindow({ conversation: conv, currentUser, onStartCall }: Props) {
  ...
  const typingUsers    = useChatStore((s) => s.typingUsers[conv._id] || []);
  ...
  useEffect(() => {
    ...
    return () => { getSocket()?.emit("room:leave", conv._id); };
  }, [conv._id]);
  ...
  const handleScroll = useCallback(async () => {
    ...
    try {
      ...
      prependMessages(conv._id, msgs);
      setPage(nextPage);
      setHasMore(msgs.length === 30);
    } catch {} finally {
      setLoadingMore(false);
    }
  }, [conv._id, hasMore, loadingMore, page, prependMessages]);

  /* ── Send message ──────────────────────────────
     TODO ②: conversationApi.sendMessage() */
  const handleSend = useCallback(async (content: string, files: File[], replyTo?: string) => {
    try {
      const res = await conversationApi.sendMessage(conv._id, { content, replyTo }, files);
  ...
  const headerName   = isGroup ? conv.name : (other?.user as User)?.fullName;
    ...
    <div className="flex flex-1 overflow-hidden">
      <div className="flex flex-col flex-1 overflow-hidden">
        ...
        <div
          ...
          style={{ background: "var(--color-surface)", borderBottom: "1px solid var(--color-s3)" }}
        >
          <div className="flex items-center gap-3">
            <div className="relative">
              <img
                src={headerAvatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(headerName || "G")}&background=e3e8f0&color=0068FF&bold=true&size=40`}
            ...
            <div>
              <p className="font-semibold text-sm" style={{ color: "var(--color-ink)" }}>{headerName}</p>
          ...
          <div className="flex items-center gap-1">
            ...
            {!isGroup && (
              <>
                <HeaderBtn title="Gọi thoại"   onClick={() => onStartCall(conv._id, "audio")}>
                  <PhoneIcon  className="w-5 h-5" />
                </HeaderBtn>
                <HeaderBtn title="Gọi video" onClick={() => onStartCall(conv._id, "video")}>
            ...
            <HeaderBtn title="Thông tin" onClick={() => setShowInfo((v) => !v)} active={showInfo}>
          ...
          {typingUsers.length > 0 && (
            <div className="flex items-center gap-2 mt-1">
              <div className="w-8 h-8 rounded-full overflow-hidden shrink-0">
                <img src={typingUsers[0].avatar || ""} className="w-full h-full object-cover" alt="" />
              </div>
              <div className="flex items-center gap-1 px-4 py-3 rounded-2xl" style={{ background: "var(--color-surface)" }}>
                <span className="dot-1 w-1.5 h-1.5 rounded-full inline-block" style={{ background: "var(--color-ink-4)" }} />
                <span className="dot-2 w-1.5 h-1.5 rounded-full inline-block" style={{ background: "var(--color-ink-4)" }} />
                <span className="dot-3 w-1.5 h-1.5 rounded-full inline-block" style={{ background: "var(--color-ink-4)" }} />
              </div>
            </div>
          )}
...
function DateDivider({ iso }: { iso: string }) {
  ...
  return (
    <div className="flex items-center gap-3 my-4">
      ...
      <span className="text-[11px] font-medium px-2" style={{ color: "var(--color-ink-4)" }}>{label}</span>
      <div className="flex-1 h-px" style={{ background: "var(--color-s4)" }} />
    </div>
  );
}
...
const VideoIcon  = ({ className }: { className: string }) => <svg className={className} fill="currentColor" viewBox="0 0 20 20"><path d="M2 6a2 2 0 012-2h6a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V6zm12.553 1.106A1 1 0 0014 8v4a1 1 0 00.553.894l2 1A1 1 0 0018 13V7a1 1 0 00-1.447-.894l-2 1z"/></svg>;
const SearchIcon = ({ className }: { className: string }) => <svg className={className} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path strokeLinecap="round" d="m21 21-4.35-4.35"/></svg>;
