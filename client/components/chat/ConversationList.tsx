...
export default function ConversationList({
  conversations, activeId, currentUser, onSelect, onCreateGroup,
}: Props) {
  ...
  return (
    <div
      ...
      style={{ width: 320, background: "var(--color-surface)", borderColor: "var(--color-s3)" }}
    >
      {/* Header */}
      <div className="px-4 pt-4 pb-2">
        <div className="flex items-center justify-between mb-3">
          <span className="font-bold text-base" style={{ color: "var(--color-ink)" }}>Tin nhắn</span>
        ...
        <div className="relative">
          <SearchIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style={{ color: "var(--color-ink-4)" }} />
          <input
            type="text"
      ...
      <div className="flex-1 overflow-y-auto">
        {filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center h-40 gap-2">
            ...
            <p className="text-sm" style={{ color: "var(--color-ink-4)" }}>Chưa có cuộc trò chuyện</p>
          </div>
        )}
        {filtered.map((c) => {
          const name   = getConvName(c, currentUser);
          const avatar = getConvAvatar(c, currentUser);
          const isOnline = c.type === "private"
          ...
          return (
            ...
            >
              ...
              <div className="relative shrink-0">
                <img
                  src={avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=e3e8f0&color=0068FF&bold=true&size=40`}
                  alt={name}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                  <span className="font-semibold text-sm truncate" style={{ color: "var(--color-ink)" }}>{name}</span>
                  <span className="text-[11px] ml-2 shrink-0" style={{ color: "var(--color-ink-4)" }}>
                    {fmtTime(c.updatedAt)}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs truncate" style={{ color: "var(--color-ink-3)" }}>
                    {getPreview(c, currentUser)}
                  </span>
                  {c.unreadCount > 0 && (
                    <span className="ml-2 min-w-[18px] h-[18px] flex items-center justify-center rounded-full text-white font-bold shrink-0 px-1"
...
function getConvName(c: Conversation, me: User | null): string {
  ...
  return (other?.user as User)?.fullName || "Người dùng";
}
